...
class Tk(tk.Tk):
    """
    A subclass of tk.Tk that allow us to assign a style manager to each window
    """
    def __init__(self):
        super(Tk, self).__init__()
        self.style_manager: 'Style' = None

    @staticmethod
    def get_style_manager(target: object):
        """ to get the style manager of a ThemedTk window
            or that of a widget"""
        while not hasattr(target, 'style_manager'):
            target = target.master
        return target.style_manager
...
class Style(ttk.Style):
    """A class for creating and managing the application
    theme and widget styles.

    This class is meant to be a drop-in replacement for `ttk.Style` and
    inherits all of it's methods and properties. Subclassing
    is not recommended and may have unintended consequences.

    Note:
    1. This class is no longer a singleton in this version.
    2. Each instance of style has a master, which must be an instance of Tk() overrided in this file.
    3. Each instance will automatically become the style_manager of its master.
    ...
    Examples:
        ...
        # instantiate the style manager of this window with default theme
        style = Style(window)

        # instantiate the style with another theme
        style = Style(window, theme='superhero')
    ...
    def __init__(self, master: Tk = None, theme=DEFAULT_THEME):
        """
        Parameters:
        ...
        self._load_themes()
        super(Style, self).__init__(master)
        master.style_manager = self  # make this style instance to be the style manager of its master.
    ...
    def configure(self, style, query_opt: Any = None, **kw):
        ...
        if not self.style_exists_in_theme(style):
            ttkstyle = Bootstyle.update_ttk_widget_style(None, style, style_manager=self)
        else:
            ttkstyle = style
    ...
    def _get_builder_tk(self):
        ...
        Returns:
            ...
            ThemeBuilderTK:
                ...
                the current theme.
        """
        builder = Style._get_builder(self)
...
class StyleBuilderTTK:
    ...
    def __init__(self, style):
        ...
        self.theme_images = {}
        self.builder_tk = StyleBuilderTK(style)
        self.create_theme()
...
class Bootstyle:
    ...
    def override_ttk_widget_constructor(func):
        ...
        def __init__(self, *args, **kwargs):
            style_object = Tk.get_style_manager(args[0])  # get the style manager of the
                                                          # window to which this widget belongs
            assert type(style_object) is Style
            ...
            if style:
                if style_object.style_exists_in_theme(style):
                    self.configure(style=style)
                else:
                    ttkstyle = Bootstyle.update_ttk_widget_style(
                        self, style, style_object, **kwargs
                    )
                    self.configure(style=ttkstyle)
            elif bootstyle:
                ttkstyle = Bootstyle.update_ttk_widget_style(
                    self, bootstyle, style_object, **kwargs
                )
                self.configure(style=ttkstyle)
            else:
                ttkstyle = Bootstyle.update_ttk_widget_style(
                    self, "default", style_object, **kwargs
                )
                self.configure(style=ttkstyle)
    ...
    def override_ttk_widget_configure(func):
        ...
        def configure(self, cnf=None, **kwargs):
            style_object = Tk.get_style_manager(self)  # get the style manager of the
                                                       # window to which this widget belongs
    ...
    def update_ttk_widget_style(
        widget: ttk.Widget = None, style_string: str = None, style_manager: Style = None, **kwargs
    ):
